...
class MujocoFetchPickAndPlaceEnv(MujocoFetchEnv, EzPickle):
    ...
    * v1: the environment depends on `mujoco_py` which is no longer maintained.
    ...
    def step(self, action):
        # 행동 적용 및 환경 상태 업데이트 로직
        observation, _, done, info,_ = super().step(action)
...
class MujocoPyFetchPickAndPlaceEnv(MujocoPyFetchEnv, EzPickle):
    def __init__(self, reward_type="sparse", **kwargs):
        ...
        EzPickle.__init__(self, reward_type=reward_type, **kwargs)
env = MujocoFetchPickAndPlaceEnv(render_mode="human")  # 또는 MujocoPyFetchPickAndPlaceEnv() 사용
rewards = []
observation,_ = env.reset()
for _ in range(100):  # 1000 스텝 동안 실행
